...
// Agility SDK (commented out for compilation)
...
ComPtr<ID3D12DescriptorHeap> CreateDescriptorHeap(ComPtr<ID3D12Device2> device, D3D12_DESCRIPTOR_HEAP_TYPE type,
...
void Resize(uint32_t width, uint32_t height) {
	if (g_ClientWidth != width || g_ClientHeight != height) {
		...
		// Flush the GPU queue to make sure the swap chain's back buffers are not
		// being referenced by an in-flight command list
...
void SetFullscreen(bool fullscreen) {
	if (g_Fullscreen != fullscreen) {
		...
		if (g_Fullscreen) {
			// Store the current window dimensions so they can be reused when switching out of the fullscreen state
			::GetWindowRect(g_hWnd, &g_WindowRect);

			// Set the window style to a borderless window so the client area fills the entire screen (remove all decorators)
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	if (g_IsInitialized) {
		switch (message) {
			...
			break;
			// The default window procedure will play a system notification sound when pressing Alt + Enter
			// if this message is not handled
			case WM_SYSCHAR:
			break;
			case WM_SIZE: {
				RECT clientRect = {};
...
int CALLBACK wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR lpCmdLine, int nCmdShow) {
	...
	// Using this awareness context allows the client area of the window to achieve 100% scaling while
	// still allowing non-client window content to be rendered in a DPI sensitive fashion.
	SetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2);

	// Window class name. Used for registering / creating the window
	const wchar_t* windowClassName = L"DX12WindowClass";
	ParseCommnadLineArguments();

	EnableDebugLayer();

	g_TearingSupported = CheckTearingSupport();
	RegisterWindowClass(hInstance, windowClassName);
	g_hWnd = CreateWindow(windowClassName, hInstance, L"DX12 Sandbox", g_ClientWidth, g_ClientHeight);

	// Initialize the globabl window rect variable
	::GetWindowRect(g_hWnd, &g_WindowRect);
